// app.js - leitura + busca + edição via função serverless
...
async function loadAll(){
  try{
    ...
    document.getElementById('results').innerText = 'Erro ao carregar dados.';
  }
}
...
async function saveJson(filename, jsonContent){
  ...
  try{
    ...
    if(resp.ok) { alert('Arquivo salvo com sucesso.'); await loadAll(); }
    else { alert('Erro ao salvar: ' + (j.error || JSON.stringify(j))); }
...
function openEditor(filename, currentJson){
  ...
  function validateJson(){
    try { JSON.parse(editorArea.value); jsonError.innerText=''; modalOk.disabled=false; }
    catch(e){ jsonError.innerText='JSON inválido: '+e.message; modalOk.disabled=true; }
  }
  editorArea.addEventListener('input', validateJson);
  validateJson();

  modalOk.onclick = async ()=>{
    try{ const parsed = JSON.parse(editorArea.value); modal.classList.add('hidden'); await saveJson(filename, parsed); }
